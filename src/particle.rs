...
pub fn maybe_spawn_particle<R: Rng + ?Sized>(
    ...
    if rng.gen_bool(0.15) {
        ...
        let (color_idx, life) = match fire_type {
            ...
            FireType::Green | FireType::Toxic => (
                (palette_len as f32 * 0.6) as u8,
                rng.gen_range(12..28),
            ),
            FireType::Purple => (
                (palette_len as f32 * 0.8) as u8,
                rng.gen_range(12..28),
            ),
            FireType::Rainbow | FireType::Candy => (
                rng.gen_range((palette_len / 2)..palette_len) as u8,
                rng.gen_range(10..30),
            ),
            FireType::FireAndIce => {
                if rng.gen_bool(0.5) {
                    ((palette_len as f32 * 0.85) as u8, rng.gen_range(10..28))
                } else {
                    ((palette_len as f32 * 0.15) as u8, rng.gen_range(14..32))
                }
            }
            FireType::ChemicalFire => (
                (palette_len as f32 * 0.7) as u8,
                rng.gen_range(10..30),
            ),
            FireType::Cyberpunk => (
            ...
            FireType::Aurora => (
                rng.gen_range((palette_len / 4)..(palette_len * 3 / 4)) as u8,
                rng.gen_range(16..36),
            ),
            FireType::Plasma => (
                if rng.gen_bool(0.3) {
                    ...
                    rng.gen_range((palette_len / 3)..palette_len) as u8
                },
                rng.gen_range(10..28),
            ),
            FireType::Void => (
...
pub fn update_particles(
    ...
    particles.retain_mut(|p| {
        if p.life > 0 && p.y > 0 {
            ...
            let dx = rand::thread_rng().gen_range(0..3) as isize - 1;
